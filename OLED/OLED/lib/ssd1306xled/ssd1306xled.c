/**
 * SSD1306xLED - Library for the SSD1306 based OLED/PLED 128x64/32 displays
 *
...
// - Partial rewrite by Kenneth Mathis in order to work with hardware I2C
// ============================================================================
...
//***PROTOTYPES***
void ssd1306_command(void);	// Initiate transmission of command
void ssd1306_data(void);	// Initiate transmission of data
...
void ssd1306_command(void) {
	i2c_start(SSD1306_SADDR); // Slave address: R/W(SA0)=0 - write
...
void ssd1306_init(void) {
	ssd1306_command();	// Initiate transmission of command
...
void ssd1306_fill4(uint8_t p1, uint8_t p2, uint8_t p3, uint8_t p4) {
	ssd1306_setpos(0, 0);
	ssd1306_data();	// Initiate transmission of data
	for (uint16_t i = 0; i < 128*32/8; i++) { //WIDTH*HEIGHT/8
...
void ssd1306_clear(void){
	ssd1306_setpos(0, 0);
	ssd1306_data();	// Initiate transmission of data
	for (uint16_t i = 0; i < 128*8/4; i++) { //WIDTH*HEIGHT/8
